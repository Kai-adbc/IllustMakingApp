...
public class ImagePanel extends Canvas implements MouseMotionListener,KeyListener,MouseListener{
	StatusManager statusManager;
	static float bairitsu = 1;
	static int X = 250, Y =30, W = 140, W2 = 114, H = 140, H2 = 114;//トリミング後画像を表示する正方形のサイズ
	int wc = 600,hc = 500;
	...
	BufferedImage next2 = new BufferedImage(W, H, BufferedImage.TYPE_INT_RGB);
	BufferedImage next3 = new BufferedImage(W, H, BufferedImage.TYPE_INT_RGB);
	BufferedImage next2Image, next3Image;
	BufferedImage nowImage,prevImage,nextImage;

	public ImagePanel(StatusManager statusManager){
		this.statusManager= statusManager;
		// MouseListener・MouseMotionListenerを設定
		addMouseListener(this);
		addKeyListener(this);
		repaint();
		setPreferredSize(new Dimension(wc, hc)); // スクロール領域のサイズを設定
    }
	...
	public void paint(Graphics g){
		...
		g.fillRect(X+W*2+A, Y+15, W2, H2);
		g.fillRect(X+W*3+A, Y+15, W2, H2);
		...
		g.drawImage(nextImage, newX+W+A, newY+15, newW2, newH2, this);
		  g.drawImage(next2Image, newX +W*2+A, newY + 15, newW2, newH2, this);
		    g.drawImage(next3Image, newX +W*3+A, newY + 15, newW2, newH2, this);
		...
		g.drawRect(X+W+A, Y+15, W2, H2);
		  // 枠線を描画
	    g.drawRect(X  +W*2+A, Y + 15, W2, H2);
	    g.drawRect(X  +W*3+A, Y + 15, W2, H2);
	...
	public void showView(){
		...
		if(-1< i){
			...
			try {prev =ImageIO.read(new File(setFilename(statusManager.event.get(i))));}
		...
		try {now =ImageIO.read(new File(setFilename(statusManager.event.get(i+1))));}
		catch (IOException e) {e.printStackTrace();}

		if(i<statusManager.event.size()-1){
			try {next =ImageIO.read(new File(setFilename(statusManager.event.get(i+2))));}
			catch (IOException e) {e.printStackTrace();}
		}else{
			Graphics2D g2d = (Graphics2D) next.createGraphics();
			g2d.fillRect(0, 0, wc, hc);
		}
		if(i < statusManager.event.size() - 2){
			try { next2 = ImageIO.read(new File(setFilename(statusManager.event.get(i+3)))); }
			catch (IOException e) { e.printStackTrace(); }
		} else {
			Graphics2D g2d = (Graphics2D) next2.createGraphics();
			g2d.fillRect(0, 0, wc, hc);
		}

		if(i < statusManager.event.size() - 3){
			try { next3 = ImageIO.read(new File(setFilename(statusManager.event.get(i+4)))); }
			catch (IOException e) { e.printStackTrace(); }
		} else {
			Graphics2D g2d = (Graphics2D) next3.createGraphics();
			g2d.fillRect(0, 0, wc, hc);
		}
		nowImage = trimImage(now);
		prevImage = trimImage(prev);
		nextImage = trimImage(next);
		next2Image = trimImage(next2);
		next3Image = trimImage(next3);
